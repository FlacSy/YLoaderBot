...
async def download_youtube(url: str, output_path: str = "downloads", message: Optional[types.Message] = None, format: str = "mp4") -> None:
    try:
        await download_youtube_with_format(url, output_path, message, format, download_format="mp4[filesize<50M]")
    except Exception as e:
        logging.error(f"Error downloading YouTube video: {str(e)}")
        logging.error(f"HTTP response: {e.response.text if getattr(e, 'response', None) else 'No response'}")

        try:
            await download_youtube_with_format(url, output_path, message, format, download_format = "18")
        except Exception as e:
            logging.error(f"Error downloading YouTube video: {str(e)}")


async def download_youtube_with_format(url: str, output_path: str, message: Optional[types.Message], format: str, download_format: str) -> None:
    options = {
