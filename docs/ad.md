# handlers/advertise_handler.py

## Модуль `advertise_handler.py`

Модуль `advertise_handler.py` содержит обработчики команд для работы с рекламными материалами. Этот модуль позволяет администраторам бота создавать, просматривать и удалять рекламные объявления.

#### Основной функционал:

1. **Создание рекламы**: Администратор может начать создание рекламы, загрузив медиа-контент и введя текст объявления.

2. **Просмотр списка рекламы**: Администратор может просмотреть список созданных рекламных объявлений.

3. **Удаление рекламы**: Администратор может выбрать рекламное объявление для удаления из списка.

4. **Включение/выключение рекламы**: Позволяет администраторам узнать текущий статус рекламы (включена или выключена).
...
async def cmd_start_advertise(message: Message):
    # Обработчик команды /start_advertise
...
async def handle_media_content(message: Message, state: FSMContext):
    # Обработчик загрузки медиа контента для рекламы
...
async def handle_ad_text(message: Message, state: FSMContext):
    # Обработчик ввода текста для рекламы
...
async def cmd_cancel(message: Message, state: FSMContext):
    # Обработчик команды отмены создания рекламы
    data = await state.get_data()
    media_path = data.get("media_path")
    if media_path:
...
async def cmd_show_ad_list(message: Message):
    # Обработчик команды просмотра списка рекламы
    user_id = message.from_user.id
    is_admin = IsAdmin(user_id).check_admin()
...
async def cmd_delete_ad(message: Message):
    # Обработчик команды удаления рекламы
    user_id = message.from_user.id
    is_admin = IsAdmin(user_id).check_admin()
...
async def delete_ad(callback_query: types.CallbackQuery):
    # Обработчик удаления рекламы по кнопке
...
async def cmd_ad_state(message: Message):
    # Обработчик команды проверки статуса рекламы
    user_id = message.from_user.id
    is_admin = IsAdmin(user_id).check_admin()
